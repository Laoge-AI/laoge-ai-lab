...
- `customer-service-demo-poster.jpg`：从用户提供的测试录屏中截取的实际画面。
...
首页的“看客服录屏”入口和客服详情页都使用 `public/media/customer-service-demo.mp4`。播放器为浏览器原生控件，支持手机内嵌播放与全屏，使用 `preload="none"` 按需加载，不自动播放。

本段约57秒，主要展示商品资料问答和连续追问。点击封面的播放按钮后使用原生播放控件；关闭JavaScript时也保留原生控件。详情页提供可展开的文字流程与已知问题说明；没有将本段描述为完整人工回复回传的演示，也不作为线上准确率证明。用户已确认按测试记录公开。

`scripts/prepare-demo.mjs` 是本地媒体处理工具，需提供FFmpeg可执行文件与原视频路径。当前录屏已检查为静音，处理时裁掉会话侧栏与窗口边框、移除静音轨、编码为H.264并启用faststart；保持提供片段的时长与顺序。FFmpeg不参与Cloudflare构建，公开仓库只需包含已处理的MP4及封面。
